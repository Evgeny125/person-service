...
public class PersonServiceImpl implements PersonService, CommandLineRunner {
	...
	public Iterable<PersonDto> findEmployeeBySalary(int min, int max) {
		return personRepository.findEmployeeBySalary(min, max).stream().map(e -> modelMapper.map(e, getDtoClass(e)))
				.collect(Collectors.toList());
	}

	@Override
	public Iterable<PersonDto> getChildren() {
		return personRepository.getChildren().stream().map(c -> modelMapper.map(c, getDtoClass(c)))
				.collect(Collectors.toList());
	}
